...
is straightforward to read the code.
...
There are of course lots of things to be improved, as we'll see below, but if
...
performed twice. In other words, we calculated the overlap statistics for each
permutation of the sets, when we really only need to calculate each combination.
...
And these improvements are essentially the advice from the [StackOverflow answer](https://stackoverflow.com/a/27369862) on the same topic.
...
variable, `overlaps_index`, to keep track of the combinations.
...
speed, so I [posted on RStudio
Community](https://community.rstudio.com/t/how-to-efficiently-calculate-pairwise-overlaps-of-many-sets/56750)
for ideas. And I was lucky to get the advice from [Nir
Graham](https://community.rstudio.com/u/nirgrahamuk/) to speed up R's set
...
needs to check during each iteration of the loop.
